...
def save_repos_to_exclude(repo_urls: set) -> None:
    ...
    all_repos = sorted(list(repos_with_errors | repo_urls))
    df_errors = pd.DataFrame({"repo_url": all_repos})
...
if __name__ == "__main__":
    ...
    # we load the previous ones such that we can ensure they are still selected again when trying to get new ones after some failed
    previous_repos = load_repos_to_include()
    print(f"Found {len(previous_repos)} previous repositories to include.")
    # we load previous repos with errors
    exclude_repos = load_repos_to_exclude()
    print(f"Found {len(exclude_repos)} previous repositories to exclude.")
